...
for index in range(weights.shape[1]-2):
    ...
    plt.hist(weights[400:, index], bins=50, alpha=0.4, facecolor='sandybrown', label='source', density=True)
    plt.hist(weights[400:, -2], bins=50, alpha=0.4, facecolor='C0', label='no-transfer', density=True)
    plt.hist(weights[400:, -1], bins=50, alpha=0.4, facecolor='C2', label='transfer', density=True)
    plt.legend()
